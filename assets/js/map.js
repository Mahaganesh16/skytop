...
$(document).ready(function(){
    ...
    $('.confirm-ride-btn').on('click',function(){
        ...
        var icons = {
            0: new google.maps.MarkerImage(
                // URL
                '../assets/front/images/taxi_icon_180.png',
                // (width,height)
            ...
            1: new google.maps.MarkerImage(
                // URL
                '../assets/front/images/taxi_icon.png',
                // (width,height)
                new google.maps.Size( 23, 23 ),
                // The origin point (x,y)
                new google.maps.Point( 0, 0 ),
            ...
            4: new google.maps.MarkerImage(
                // URL
                '../assets/front/images/taxi_icon_270.png',
                // (width,height)
                new google.maps.Size( 23, 23 ),
                // The origin point (x,y)
                new google.maps.Point( 0, 0 ),
            ...
            6: new google.maps.MarkerImage(
                // URL
                '../assets/front/images/taxi_icon_90.png',
                // (width,height)
                new google.maps.Size( 23, 23 ),
                // The origin point (x,y)
                new google.maps.Point( 0, 0 ),
        ...
        var markerList = [{lat: 9.931469,lng: 78.094223},
            {lat: 9.931579,lng: 78.096340},
            {lat: 9.931775,lng: 78.098223},
            {lat: 9.931469,lng: 78.094223},
    ...
    $(document).on('click', '.tapped-car-info .slide-up', function(){
        ...
        $(this).find('img').attr('src','../assets/front/icon/down-arrow.svg');
    ...
    $(document).on('click', '.tapped-car-info .slide-down', function(){
        ...
        $(this).find('img').attr('src','../assets/front/icon/up-arrow.svg');
    ...
    $("#accept_request").on('click',function(){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                window.location.href = "<?php echo base_url('worker/Dashboard')?>";
                  $('.tapped-car-info').addClass('hidden');
            $('.map-notification').addClass('hidden');
            multipleRoute(new google.maps.DirectionsService, new google.maps.DirectionsRenderer,origin,destination);
            $('.go-to-pickup-btn').removeClass('hidden');
          },error: function(){
            alert('Error');
          } 
    ...
    $(document).on('click','.request-ride-info .slide-up', function(){
        ...
        if($(this).hasClass('double-up')){
            $(this).find('img').attr('src', '../assets/front/icon/downarrow.svg');
            $(this).removeClass('double-up').addClass('double-down');
        }
        else{
            $(this).find('img').attr('src', '../assets/front/icon/uparrow.svg');
...
function multipleRoute(directionsService, directionsDisplay,origin,destination){
// alert(origin);
    ...
    var icons = {
        ...
        marker: new google.maps.MarkerImage(
            // URL
            '../assets/front/icon/red-marker.svg',
            // (width,height)
    ...
    makeMarker(origin, icons.end, "Route End");
    for(var i=0;i<coordinates.length;i++){
        makeMarker(coordinates[i], icons.marker,"Route Stop");
    }
    showMapRequestPin(origin);
}
...
function initMap() {
    map = new google.maps.Map(document.getElementById('map'), {
        ...
        zoom: 19,
        icon:'https://www.olacabs.com/webstatic/img/ola-logo.svg',
...
//         map: map,
//         icon: './assets/front/icon/clocation.svg',
//         draggable:true
//     });
//   google.maps.event.addListener(marker,'dragend',function(evt){
//          var lat = evt.latLng.lat().toFixed(9);
//         var long =  evt.latLng.lng().toFixed(9);
//           // alert(lat);
//           $.ajax({
//             type: "POST",
//             url: "./Dashboard/UpdateCurrentLocation",
...
function showMapRequestPin(origin){
    var icons = {
        request: new google.maps.MarkerImage(
            // URL
            '../assets/front/icon/car-start-position.svg',
            // (width,height)
    ...
    makeMarker( origin, icons.request, "Request Pin", false);
    setTimeout(function() {
        if (!$("img[src='../assets/front/icon/car-start-position.svg']").parent().hasClass('pulse')) {
            $("img[src='../assets/front/icon/car-start-position.svg']").parent().addClass('pulse').css('opacity',1);
        }
    },600);
}
...
AutocompleteDirectionsHandler.prototype.route = function() {
     var startpoint = $('#origin-input').val();
     var endpoint = $('#destination-input').val();
            ...
            if (status === 'OK') {
                var icons = {
                    start: new google.maps.MarkerImage(
                        // URL
                        'https://cdn0.iconfinder.com/data/icons/taxi-service-5/512/479_Customer_Location_Map_Marker_Person-128.png',
                        // (width,height)
                ...
                setTimeout(function() {
                    var currents = $("#map img[src='../assets/front/icon/circle.svg']").parent();
