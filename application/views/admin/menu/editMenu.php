					...
					<div class="col-md-12">  				
	          			<div class="col-md-6">
				            <div class="card card-primary">
					            ...
					            <form role="form" method="POST" action="<?= base_url('admin/Menus/editMenu/'.$menuDetail->id);?>">
					                <div class="card-body">
					                  	<div class="form-group">
					                    	<label>Parent Menu</label>
					                    	<select class="form-control select2" name="parent" id="parent">
					                    		<option value="">Select</option>
					                    		<option value="0" <?php if($menuDetail->parent == '0') echo "selected";?>>Main Menu</option>
					                    		<?php
					                    		if(!empty($menus)){
					                    		foreach ($menus as $menu) {
					                    		?>
					                    		<option value="<?php echo $menu->id;?>" <?php if($menu->id == $menuDetail->parent) echo "selected";?>><?php echo $menu->menu; ?></option>
					                    		<?php
					                    		}
					                    		}
					                    		?>
					                  	...
					                  	<div class="form-group row">
					                  		<div class="col-md-6">
						                    	<label>Menu</label>
						                    	<input type="text" class="form-control" id="menu" name="menu" placeholder="Menu" value="<?php echo $menuDetail->menu;?>">
						                    	<?php echo form_error('menu',"<div style='color:red'>","</div>");?>
						                    </div>
						                    <div class="col-md-6">
						                    	<label>Slug</label>
						                    	<input type="text" class="form-control" id="slug" name="slug" placeholder="Slug" readonly value="<?php echo $menuDetail->slug;?>">
						                    	<?php echo form_error('slug',"<div style='color:red'>","</div>");?>
						                    </div>
					                  	</div>
					                  	<div class="form-group">
					                    	<label>URL</label>
					                    	<input type="text" class="form-control" id="url" name="url" placeholder="URL" value="<?php echo $menuDetail->url;?>">
					                  	...
					                  	<div class="form-group">
					                    	...
					                    	<select class="form-control" id="status" name="status">
					                    		<option value="">Select</option>
					                    		<option value="1" <?php if($menuDetail->status == '1') echo "selected";?>>Active</option>
					                    		<option value="0" <?php if($menuDetail->status == '0') echo "selected";?>>In Active</option>
					                    	</select> 
